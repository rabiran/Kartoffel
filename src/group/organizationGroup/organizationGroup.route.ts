...
import {
 extractor as personScopePolicyExtractor,
 postTransformer as personScopePolicyResultTransformer
} from '../../person/scopePolicy';

// import { body, param, check, validationResult } from 'express-validator/check';

const organizationGroups = Router();

organizationGroups.get('/', ch(
  OrganizationGroup.getOrganizationGroups, 
...
organizationGroups.get(
  ...
  ch(
    OrganizationGroup.getUpdatedFrom, 
    (req: Request) => {
...
ch(
  OrganizationGroup.getOrganizationGroupByHierarchy, 
  (req: Request) => {
...
ch(
  OrganizationGroup.getOrganizationGroupByAkaUnit, 
  (req: Request) => {
...
organizationGroups.get('/:id/members', 
  validatorMiddleware(Vld.validMongoId, ['id'], 'params'),
  ch(
    OrganizationGroup.getAllMembers, 
    (req: Request, res: Response) => [req.params.id],
    personScopePolicyExtractor,
    personScopePolicyResultTransformer
  )
);
...
organizationGroups.get('/:id/children', validatorMiddleware(Vld.validMongoId, ['id'], 'params'), 
  ...
  ch(
    OrganizationGroup.getOffsprings, 
    (req: Request) => [req.params.id, Number(req.query.maxDepth)],
    scopePolicyExtractor,
    scopePolicyResultTransformer
  )
);
